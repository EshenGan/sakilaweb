<?php
    include_once ('connect.php');
    
$filmid = $_GET['filmid'];
    $result = mysqli_query($conn,"SELECT * FROM FILM WHERE FILM_ID='$filmid'");
    $rows = mysqli_fetch_array($result);
    
    if(isset($_POST['updatefilm']))
    {
        $ry = $_POST['ry'];
        $languageid = $_POST['languageid'];
        $olid = $_POST['olid'];
        $rating = $_POST['rating'];
        $sf = $_POST['sf'];
        $lastupdate = $_POST['lastupdate'];
        
        
        $update = mysqli_query($conn,"UPDATE FILM SET RELEASE_YEAR='$ry', LANGUAGE_ID='$languageid', ORIGINAL_LANGUAGE_ID='$olid', RATING='$rating', SPECIAL_FEATURES='$sf', LAST_UPDATE='$lastupdate' WHERE FILM_ID='$filmid';");
        
        if ($update)
        {
            header("refresh:1; url=film.php");
        }
        else
        {
            echo "Not updated";
        }
    }
?>
	
	<head>
		<title> SAKILA | ADDRESS UPDATE</title>
		<meta charset="UTF-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1" />
		<link rel="shortcut icon" type="image/x-icon" href="logo_icon.jpg" />
		<link rel="stylesheet" href="styles.css" text="text/css"/>
		<link rel="stylesheet" href="fontawesomemin.css"/>
		<link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
		<script src="querymin.js"></script>
		<script src="bootstrapmin.js"></script>
	</head>
	
	<body>
	    <table border="0" align="center" style="text-decoration:none" >
					<tr><td>
				   <img class="container-fluid" id="icon" src="logo_icon.jpg" alt="SAKILA"  type="image" width="250vw" height="200vw"
				    title="SAKILA" align="center"/>
					<br />
					</td></tr>
		</table>
		
		<br>
     	<table border="0" style="width: 100%">
     		<tr style="background-color:#2966c2">
     		<td>
     		<h1 id="header" style="color: #FFFFFF" align="center">UPDATE DATA HERE</h1>
     		</td>
     		</tr>
 		</table>
     	<br>
		
					
		<table id="Demo" style="background-color: powderblue;" align="center">
			<tr><form method="POST">
			    <td>Release Year<input type="text" name="ry" value="<?php echo $rows['RELEASE_YEAR'] ?>"</td>
		        <td>Language ID<input type="text" name="languageid" value="<?php echo $rows['LANGUAGE_ID'] ?>"</td>
			    <td>Original Language ID<input type="text" name="olid" value="<?php echo $rows['ORIGINAL_LANGUAGE_ID'] ?>"</td>
			</tr>
			<tr>
			    <td>Rating<input type="text" name="rating" value="<?php echo $rows['RATING'] ?>"</td>
			    <td>Special Features<input type="text" name="sf" value="<?php echo $rows['SPECIAL_FEATURES'] ?>"</td>
			    <td>Last Update<input type="text" name="lastupdate" value="<?php echo $rows['LAST_UPDATE'] ?>"</td>
			    <td><input type="submit" name="updatefilm" value="UPDATE"></td>
			</form></tr>
      	</table>
    </body> 