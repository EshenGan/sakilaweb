<?php
    ...
    $faid = $_GET['filmactorid'];
    $result = mysqli_query($conn,"SELECT * FROM FILM_ACTOR WHERE ACTOR_ID='$faid'");
    $rows = mysqli_fetch_array($result);
    
    if(isset($_POST['updatefilmactor']))
    {
        $filmid = $_POST['filmid'];
        $lastupdate = $_POST['lastupdate'];
        
        
        $update = mysqli_query($conn,"UPDATE FILM_ACTOR SET FILM_ID='$filmid',LAST_UPDATE='$lastupdate' WHERE ACTOR_ID='$faid';");
...
?>
	...
	<head>
		<title> SAKILA | FILM_ACTOR UPDATE</title>
		<meta charset="UTF-8" />
	...
	<body>
		...
		<table id="Demo" style="background-color: powderblue;" align="center">
			<tr><form method="POST">
			    ...
			    <td><input type="submit" name="updatefilmactor" value="UPDATE">
			</form></td>
      	</table>
    </body> 